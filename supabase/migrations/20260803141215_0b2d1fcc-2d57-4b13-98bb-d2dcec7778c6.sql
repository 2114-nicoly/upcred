-- 1) Bloquear fechamentos antigos sem snapshot
REVOKE EXECUTE ON FUNCTION public.close_daily_cash(date) FROM PUBLIC, anon, authenticated;
...
REVOKE EXECUTE ON FUNCTION public.close_daily_cash_with_snapshot(date, numeric, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.close_daily_cash_with_snapshot(date, numeric, text) TO authenticated, service_role;

-- 3) Bloquear snapshot enviado diretamente
DROP POLICY IF EXISTS snapshot_insert_scoped ON public.daily_cash_snapshots;
DROP POLICY IF EXISTS snapshot_update_scoped ON public.daily_cash_snapshots;
REVOKE INSERT, UPDATE, DELETE ON public.daily_cash_snapshots FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.daily_cash_snapshots TO authenticated, anon;
GRANT ALL ON public.daily_cash_snapshots TO service_role;

-- 2) Garantia definitiva no daily_cash (constraint trigger DEFERRABLE)
CREATE OR REPLACE FUNCTION public.daily_cash_require_snapshot()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  IF NEW.status = 'closed' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.daily_cash_snapshots s
       WHERE s.daily_cash_id = NEW.id
         AND s.closed_at >= COALESCE(NEW.closed_at, now())
    ) THEN
...
DROP TRIGGER IF EXISTS trg_daily_cash_require_snapshot ON public.daily_cash;
CREATE CONSTRAINT TRIGGER trg_daily_cash_require_snapshot
  AFTER INSERT OR UPDATE ON public.daily_cash
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  WHEN (NEW.status = 'closed')
  EXECUTE FUNCTION public.daily_cash_require_snapshot();

-- 4) Motivo de reabertura vinculado ao caixa
...
-- 5 e 6) "Não pagou" com escopo completo + saldo devedor por empréstimo
...
DECLARE
  ...
  v_np_total int := 0; v_np_valid int := 0;
  v_payload jsonb;
BEGIN
  ...
  -- Não pagamentos (escopo completo: parcela -> empréstimo -> cliente)
  SELECT COUNT(*)::int INTO v_np_total
  ...
  SELECT COALESCE(jsonb_agg(
           to_jsonb(nm) || jsonb_build_object('installment',
             to_jsonb(i) || jsonb_build_object('loans',
               to_jsonb(l) || jsonb_build_object('clients',
                 jsonb_build_object('id', c.id, 'name', c.name))))
         ), '[]'::jsonb),
         COUNT(*)::int
    INTO v_np, v_np_valid
    FROM public.not_paid_marks nm
    JOIN public.installments i ON i.id = nm.installment_id AND i.loan_id = nm.loan_id
  ...
  -- Clientes atrasados: saldo devedor somado UMA vez por empréstimo
  WITH base AS (
    SELECT l.client_id, c.name AS client_name, l.worker_id,
           l.id AS loan_id, l.remaining_balance,
           i.id AS inst_id, i.number, i.amount,
           COALESCE(i.paid_amount,0) AS paid_amount, i.due_date
      FROM public.installments i
      JOIN public.loans l ON l.id = i.loan_id
      JOIN public.clients c ON c.id = l.client_id
     WHERE i.due_date < v_date
  ...
  per_loan AS (
    SELECT DISTINCT client_id, loan_id, remaining_balance FROM base
  ),
  grouped AS (
    SELECT b.client_id, b.client_name, b.worker_id,
           COUNT(*)::int AS cnt,
           SUM(GREATEST(0, b.amount - b.paid_amount)) AS total,
           MIN(b.due_date) AS oldest,
           (SELECT COALESCE(SUM(pl.remaining_balance),0) FROM per_loan pl WHERE pl.client_id = b.client_id) AS remaining,
           jsonb_agg(jsonb_build_object(
             'installment_id', b.inst_id, 'loan_id', b.loan_id, 'number', b.number,
             'amount', b.amount, 'paid_amount', b.paid_amount,
             'pending_amount', GREATEST(0, b.amount - b.paid_amount),
             'due_date', b.due_date,
             'overdue_days', GREATEST(0, (v_date - b.due_date))
           ) ORDER BY b.due_date) AS installments
      FROM base b
     GROUP BY b.client_id, b.client_name, b.worker_id
  )
