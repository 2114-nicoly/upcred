CREATE OR REPLACE FUNCTION public.get_active_daily_cash_for_scope(
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    -- Trabalhador: somente o próprio caixa, dentro da própria empresa.
  ...
  ELSIF v_is_super THEN
    ...
    v_admin  := COALESCE(v_worker_admin, p_admin_id, v_caller_admin);
    IF p_worker_id IS NOT NULL AND p_admin_id IS NOT NULL
       AND v_worker_admin IS DISTINCT FROM p_admin_id THEN
      RAISE EXCEPTION 'trabalhador não pertence à empresa selecionada';
    END IF;
  ELSE
    -- Administrador: apenas a própria empresa e seus trabalhadores.
...
REVOKE ALL ON FUNCTION public.get_active_daily_cash_for_scope(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_active_daily_cash_for_scope(uuid, uuid) TO authenticated, service_role;