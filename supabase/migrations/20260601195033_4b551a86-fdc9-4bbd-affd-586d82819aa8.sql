...
-- Fix _daily_cash_is_empty to be tolerant with admin_id null in old records.
-- Match daily_events / cash_movements / not_paid_marks / loans by (cash_date, worker_id),
-- ignoring admin_id mismatch when either side is null.
...
BEGIN
  ...
  -- daily_events except 'caixa_aberto'
...
-- Tolerant find: admin filter also accepts dc.admin_id NULL when worker belongs to that admin
...
-- Tolerant cleanup with same admin null tolerance
